...
public class ChessClient extends JFrame {
	...
	public ChessClient() {
		...
		// use awt.Toolkit to get screensize and set xPos and yPos to the center of the
		// screen
		...
		// use removeMinMaxClose method to keep user from closing window with X but
		// still be able to drag window
	...
	private static void getIPAndConnect() throws InterruptedException {
		...
		/*
		 * this do-while loop asks to reenter ip info if connection to the server fails
		 */
		do {
			...
			/*
			 * The following loop makes the client wait until AddressFrame passes the IP
			 * info to client
			 */
			while (!ipEntered) {
				Thread.sleep(50);
			...
			/*
			 * Use try-catch to try to connect to RMI objects, if it fails, then have the
			 * user reenter info
			 */
			try {
	...
	private static void promptSessions() throws InterruptedException, RemoteException {
		...
		do {
			do {
				/*
				 * The following loop makes the client wait until sessionPanel passes the game
				 * session number
				 */
	...
	public static void enableMoves() throws RemoteException {
		...
		if (selectionX >= 0 && selectionX <= 7 && selectionY >= 0 && selectionY <= 7) {
			...
			if (playerColor == 1) {
				if (!(j == 0)
						&& Objects.equals(sesh[gameSessionSelected].getPieceTypeAt(selectionX, selectionY), "Pawn")) {
	...
	public static void knightEnable(int i, int j) throws RemoteException {
		...
		i = selectionX + 2;
		j = selectionY + 1;
		if (i <= 7 && j <= 7 && (Objects.equals(sesh[gameSessionSelected].getPieceTypeAt(i, j), "BlankPiece")
				|| sesh[gameSessionSelected].getColorAt(i, j) != playerColor)) {
			buttonArray[i][j].setEnabled(true);
			moves++;
		}
		...
		i = selectionX - 2;
		j = selectionY - 1;
		if (i >= 0 && j >= 0 && (Objects.equals(sesh[gameSessionSelected].getPieceTypeAt(i, j), "BlankPiece")
				|| sesh[gameSessionSelected].getColorAt(i, j) != playerColor)) {
			buttonArray[i][j].setEnabled(true);
			moves++;
		}
		...
		i = selectionX - 2;
		j = selectionY + 1;
		if (i >= 0 && j <= 7 && (Objects.equals(sesh[gameSessionSelected].getPieceTypeAt(i, j), "BlankPiece")
				|| sesh[gameSessionSelected].getColorAt(i, j) != playerColor)) {
			buttonArray[i][j].setEnabled(true);
			moves++;
		}
		...
		i = selectionX + 2;
		j = selectionY - 1;
		if (i <= 7 && j >= 0 && (Objects.equals(sesh[gameSessionSelected].getPieceTypeAt(i, j), "BlankPiece")
				|| sesh[gameSessionSelected].getColorAt(i, j) != playerColor)) {
			buttonArray[i][j].setEnabled(true);
			moves++;
		}
	...
	public static void kingEnable(int i, int j) throws RemoteException {
		i = selectionX + 1;
		j = selectionY;
		if (i <= 7 && (Objects.equals(sesh[gameSessionSelected].getPieceTypeAt(i, j), "BlankPiece")
				|| sesh[gameSessionSelected].getColorAt(i, j) != playerColor)) {
			buttonArray[i][j].setEnabled(true);
			moves++;
		}
		...
		i = selectionX;
		j = selectionY + 1;
		if (j <= 7 && (Objects.equals(sesh[gameSessionSelected].getPieceTypeAt(i, j), "BlankPiece")
				|| sesh[gameSessionSelected].getColorAt(i, j) != playerColor)) {
			buttonArray[i][j].setEnabled(true);
			moves++;
		}
		...
		i = selectionX - 1;
		j = selectionY;
		if (i >= 0 && (Objects.equals(sesh[gameSessionSelected].getPieceTypeAt(i, j), "BlankPiece")
				|| sesh[gameSessionSelected].getColorAt(i, j) != playerColor)) {
			buttonArray[i][j].setEnabled(true);
			moves++;
		}
		...
		i = selectionX;
		j = selectionY - 1;
		if (j >= 0 && (Objects.equals(sesh[gameSessionSelected].getPieceTypeAt(i, j), "BlankPiece")
				|| sesh[gameSessionSelected].getColorAt(i, j) != playerColor)) {
			buttonArray[i][j].setEnabled(true);
			moves++;
		}
